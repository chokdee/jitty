...
create table tournament_class (id bigint generated by default as identity (start with 1), end_ttr integer, game_mode_phase_1 varchar(1), game_mode_phase_2 varchar(1), group_count integer, max_age date, min_age date, name varchar(255) not null, open_for_men boolean not null, open_for_women boolean not null, phase integer, player_per_group integer, quali_group_count integer not null, running boolean not null, start_ttr integer, start_time timestamp, type varchar(255), KOFIELD_ID bigint, T_ID bigint, primary key (id));
create table tournament_group (id bigint generated by default as identity (start with 1), name varchar(10) not null, TC_ID bigint, primary key (id));
...
alter table tournament_class add constraint FKn3c4yk1kcc1u80xlabchc9mt6 foreign key (KOFIELD_ID) references ko_field;
alter table tournament_class add constraint FK78e4gyb2n3nahl2de4ex9c9qp foreign key (T_ID) references tournament;
alter table tournament_group add constraint FKg793y9b2sgkh8po179ah44w4a foreign key (TC_ID) references tournament_class;
