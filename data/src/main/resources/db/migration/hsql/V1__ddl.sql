...
create table club (id bigint generated by default as identity (start with 1), click_tt_nr varchar(255), name varchar(255) not null, association_id bigint, primary key (id));
...
create table Phase (id bigint generated by default as identity (start with 1), S_ID bigint, INDEXP integer, primary key (id));
...
create table swiss_system_phase (MAX_ROUNDS integer not null, ROUND integer not null, id bigint not null, group_id bigint, primary key (id));
create table T_PLAYER (Tournament_id bigint not null, players_id bigint not null, primary key (Tournament_id, players_id));
create table t_system (id bigint generated by default as identity (start with 1), TC_ID bigint, primary key (id));
create table table_pos (id bigint generated by default as identity (start with 1), column smallint, row smallint, SETTINGS_ID bigint, primary key (id));
create table table_settings (id bigint generated by default as identity (start with 1), table_count integer, T_ID bigint, primary key (id));
...
create table tournament (id bigint generated by default as identity (start with 1), click_tt_id varchar(255), end_date date not null, name varchar(255) not null, running boolean not null, start_date date not null, table_count integer not null, primary key (id));
create table tournament_class (id bigint generated by default as identity (start with 1), ACTIVE_PHASE integer not null, age_group varchar(255) not null, end_ttr integer, max_age date, min_age date, name varchar(255) not null, open_for_men boolean not null, open_for_women boolean not null, RUNNING boolean not null, start_ttr integer, START_TIME timestamp, status varchar(255) not null, SYSTEM_TYPE integer not null, T_ID bigint, primary key (id));
...
create table tournament_player (id bigint generated by default as identity (start with 1), birthday date, click_tt_internal_nr varchar(255), click_tt_licence_nr varchar(255), email varchar(255), firstname varchar(255) not null, gender varchar(1), last_game_at timestamp, lastname varchar(255) not null, mobilenumber varchar(255), qttr integer, suspended boolean, ttr integer, association_id bigint, club_id bigint, tournament_id bigint, primary key (id));
create table TOURNAMENT_SINGLE_GAME (id bigint generated by default as identity (start with 1), called boolean not null, end_time timestamp, NAME varchar(255), played boolean not null, start_time timestamp, table_no integer, tournament_class_name varchar(255) not null, t_id bigint not null, win_by_default boolean, win_reason integer, winner integer not null, GAME_QUEUE_ID bigint, group_id bigint, NEXT_GAME_ID bigint, player1_id bigint, player2_id bigint, ROUND_ID bigint, primary key (id));
...
alter table T_PLAYER add constraint UK_a8o22qx76ytwm779shvhkhd9r unique (players_id);
...
alter table Phase add constraint FKb8sbbbk5ai0d5o0w1oiu2fcd0 foreign key (S_ID) references t_system;
...
alter table T_PLAYER add constraint FKox4dt24cvyx5jdn4rs9lyeyk5 foreign key (players_id) references tournament_player;
alter table T_PLAYER add constraint FK9sdl5mre08eax08oau8wfudro foreign key (Tournament_id) references tournament;
alter table t_system add constraint FKer0y033wsm6ysyosohkxlykss foreign key (TC_ID) references tournament_class;
alter table table_pos add constraint FKqv40g8sy1drhdvgm1o6qr6h3h foreign key (SETTINGS_ID) references table_settings;
alter table table_settings add constraint FKghas75wtmj0dkjkm1hovh2r1j foreign key (T_ID) references tournament;
...
alter table tournament_player add constraint FKrrm3jbmm1fxx5t9t5f8t46ebc foreign key (tournament_id) references tournament;
